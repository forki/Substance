...
namespace System.Collections.Generic
{
    ...
    public class SortedSet<T> : ISet<T>, ICollection<T>, ICollection, IReadOnlyCollection<T>
    {
        ...
        internal virtual bool AddIfNotPresent(T item)
        {
            ...
            Debug_.Assert(parent != null, "Parent node cannot be null here!");
                ...
                { // fix up 2-Node
                    ...
                    else
                    {
                        ...
                        if (sibling.IsRed)
                        {
                            ...
                            Debug_.Assert(!parent.IsRed, "parent must be a black node!");
                        ...
                        Debug_.Assert(sibling != null || sibling.IsRed == false, "sibling must not be null and it must be black!");
                        ...
                        else
                        {
                            ...
                            switch (rotation)
                            {
                                case TreeRotation.RightRotation:
                                    Debug_.Assert(parent.Left == sibling, "sibling must be left child of parent!");
                                    Debug_.Assert(sibling.Left.IsRed, "Left child of sibling must be red!");
                                    sibling.Left.IsRed = false;
                                    newGrandParent = RotateRight(parent);
                                    break;
                                case TreeRotation.LeftRotation:
                                    Debug_.Assert(parent.Right == sibling, "sibling must be left child of parent!");
                                    Debug_.Assert(sibling.Right.IsRed, "Right child of sibling must be red!");
        ...
        {
            Debug_.Assert(grandParent != null, "Grand parent cannot be null here!");
        ...
        {
            Debug_.Assert(node != null, "node cannot be null!");
        ...
        {
            Debug_.Assert(IsRed(parent), "parent must be be red");
        ...
        private void ReplaceNode(Node match, Node parentOfMatch, Node succesor, Node parentOfSuccesor)
        {
            ...
            {  // this node has no successor, should only happen if right child of matching node is null.
                Debug_.Assert(match.Right == null, "Right child must be null!");
                succesor = match.Left;
            }
            else
            {
                Debug_.Assert(parentOfSuccesor != null, "parent of successor cannot be null!");
                Debug_.Assert(succesor.Left == null, "Left child of succesor must be null!");
                Debug_.Assert((succesor.Right == null && succesor.IsRed) || (succesor.Right.IsRed && !succesor.IsRed), "Succesor must be in valid state");
        ...
        {
            Debug_.Assert(IsRed(sibling.Left) || IsRed(sibling.Right), "sibling must have at least one red child");
        ...
        internal sealed class TreeSubSet : SortedSet<T>
        {
            ...
            internal override bool AddIfNotPresent(T item)
            {
                ...
                VersionCheck();
#if DEBUG
                Debug_.Assert(this.versionUpToDate() && _root == _underlying.FindRange(_min, _max));
#endif
            ...
            {
                Debug_.Assert(_underlying != null, "Underlying set no longer exists");
